...
class Manufacturer extends \Opencart\System\Engine\Model {
	...
	public function getManufacturers(array $data = []): array {
		...
		if (!empty($data['filter_name'])) {
			$filter_name = oc_strtolower((string)$data['filter_name']);

			if (!str_contains($filter_name, '%')) {
				$filter_name = '%' . $filter_name . '%';
			} else {
				if (!str_starts_with($filter_name, '%')) {
					$filter_name = '%' . $filter_name;
				}

				if (!str_ends_with($filter_name, '%')) {
					$filter_name .= '%';
				}
			}

			$sql .= " WHERE LCASE(`name`) LIKE '" . $this->db->escape($filter_name) . "'";
		}
