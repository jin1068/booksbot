...
class Category extends \Opencart\System\Engine\Model {
	/**
	 * Get children categories for a given parent.
	 *
	 * @param int $parent_id
	 *
	 * @return array<int, array<string, mixed>>
	 */
	public function getChildren(int $parent_id = 0): array {
		$sql = "SELECT c.`category_id`, c.`parent_id`, c.`sort_order`, c.`status`, c.`image`, cd.`name`, (SELECT COUNT(*) FROM `" . DB_PREFIX . "category` child WHERE child.`parent_id` = c.`category_id`) AS `children` FROM `" . DB_PREFIX . "category` c LEFT JOIN `" . DB_PREFIX . "category_description` cd ON (c.`category_id` = cd.`category_id`) WHERE cd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND c.`parent_id` = '" . (int)$parent_id . "' ORDER BY c.`sort_order`, cd.`name`";
	...
	public function getCategories(array $data = []): array {
		...
		if (!empty($data['filter_name'])) {
			$raw_filter_name = (string)$data['filter_name'];
			$filter_name = oc_strtolower($raw_filter_name);
			$filter_like = $filter_name;

			if (!str_contains($filter_like, '%')) {
				$filter_like = '%' . $filter_like . '%';
			} else {
				if (!str_starts_with($filter_like, '%')) {
					$filter_like = '%' . $filter_like;
				}

				if (!str_ends_with($filter_like, '%')) {
					$filter_like .= '%';
				}
			}

			$sql .= " AND LCASE(`cd2`.`name`) LIKE '" . $this->db->escape($filter_like) . "'";
		}
		...
		if ($raw_filter_name !== '') {
			...
			$words = explode(' ', trim(preg_replace('/\s+/', ' ', str_ireplace([' &gt; ', ' > '], ' ', oc_strtolower(str_replace('%', ' ', $raw_filter_name))))));
			...
			if ($implode) {
				$sql .= " HAVING ((" . implode(" AND ", $implode) . ") OR LCASE(`name`) LIKE '" . $this->db->escape($filter_like ?: oc_strtolower($raw_filter_name)) . "')";
			}
		}
