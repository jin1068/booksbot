...
class Product extends \Opencart\System\Engine\Model {
	...
	public function addProduct(array $data): int {
		...
		if (!empty($data['product_variant'])) {
			foreach ($data['product_variant'] as $product_variant) {
				$this->model_catalog_product->addProductVariant($product_id, $product_variant + ['product_id' => $product_id]);
			}
		}
	...
	public function editProduct(int $product_id, array $data): void {
		...
		$this->model_catalog_product->deleteProductVariants($product_id);
	...
	public function addVariant(int $master_id, array $data): int {
		...
		if ($master_info) {
			...
			if (!isset($override['product_variant'])) {
				$product_data['product_variant'] = $this->model_catalog_product->getProductVariants($master_id);

				foreach ($product_data['product_variant'] as $index => $variant) {
					$product_data['product_variant'][$index]['variant_id'] = 0;
				}
			}
		}
	...
	public function getProduct(int $product_id): array {
		...
		if ($query->num_rows) {
			...
			$product_data['product_variant'] = $this->model_catalog_product->getProductVariants($product_id);
		}
	...
	if (isset($data['filter_image']) && $data['filter_image'] !== '') {
		if ($data['filter_image'] == '1') {
			...
			$sql .= " AND `p`.`image` NOT LIKE '%placeholder%'";
			$sql .= " AND `p`.`image` NOT LIKE '%no_image%'";
			$sql .= " AND `p`.`image` NOT LIKE 'catalog/demo/%'";
		} elseif ($data['filter_image'] == '0') {
			// No image (包括空值、NULL和占位图)
			$sql .= " AND (`p`.`image` = '' OR `p`.`image` IS NULL";
			$sql .= " OR `p`.`image` LIKE '%placeholder%'";
			$sql .= " OR `p`.`image` LIKE '%no_image%'";
			$sql .= " OR `p`.`image` LIKE 'catalog/demo/%')";
		}
	}
	 ...
	 */
	public function addProductVariant(int $product_id, array $data): int {
		$sku = $data['sku'] ?? null;
		$model = $data['model'] ?? '';
		...
		$image = $data['image'] ?? ($data['images'][0]['image'] ?? '');

		$this->db->query("INSERT INTO `" . DB_PREFIX . "product_variant` SET `product_id` = '" . (int)$product_id . "', `sku` = " . ($sku !== null ? "'" . $this->db->escape((string)$sku) . "'" : "NULL") . ", `model` = '" . $this->db->escape((string)$model) . "', `price` = '" . $price . "', `quantity` = '" . $quantity . "', `image` = '" . $this->db->escape((string)$image) . "', `weight` = '" . $weight . "', `status` = '" . $status . "', `sort_order` = '" . $sort_order . "'");

		$variant_id = (int)$this->db->getLastId();

		if (!empty($data['options'])) {
			$this->model_catalog_product->addProductVariantOptions($variant_id, (array)$data['options']);
		}

		if (!empty($data['images'])) {
			foreach ($data['images'] as $variant_image) {
				$this->model_catalog_product->addProductVariantImage($variant_id, $variant_image);
			}
		}
	 ...
	 * @return void
	 */
	public function deleteProductVariants(int $product_id): void {
		$query = $this->db->query("SELECT `variant_id` FROM `" . DB_PREFIX . "product_variant` WHERE `product_id` = '" . (int)$product_id . "'");

		foreach ($query->rows as $row) {
			$variant_id = (int)$row['variant_id'];

			$this->model_catalog_product->deleteProductVariantOptions($variant_id);
			$this->model_catalog_product->deleteProductVariantImages($variant_id);
		}
	 ...
	 * @param array $option_value_ids      option value IDs
	 *
	 * @return void
	 */
	public function addProductVariantOptions(int $variant_id, array $option_value_ids): void {
		foreach ($option_value_ids as $option_value_id) {
			if ($option_value_id === '' || $option_value_id === null) {
				continue;
			}

			$this->db->query("INSERT INTO `" . DB_PREFIX . "product_variant_option` SET `variant_id` = '" . (int)$variant_id . "', `product_option_value_id` = '" . (int)$option_value_id . "'");
		}
	}

	/**
	 * Delete Product Variant Options
	 *
	 * @param int $variant_id variant ID
	 *
	 * @return void
	 */
	public function deleteProductVariantOptions(int $variant_id): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_variant_option` WHERE `variant_id` = '" . (int)$variant_id . "'");
	}
	 ...
	 * @return void
	 */
	public function addProductVariantImage(int $variant_id, array $data): void {
		if (empty($data['image'])) {
			return;
		}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "product_variant_image` SET `variant_id` = '" . (int)$variant_id . "', `image` = '" . $this->db->escape((string)$data['image']) . "', `sort_order` = '" . (int)($data['sort_order'] ?? 0) . "'");
	}
	 ...
	 * @return void
	 */
	public function deleteProductVariantImages(int $variant_id): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_variant_image` WHERE `variant_id` = '" . (int)$variant_id . "'");
	}
	...
	public function getProductVariants(int $product_id): array {
		...
		foreach ($query->rows as $row) {
			...
			$variant_data[] = [
				...
				'options'    => $this->model_catalog_product->getProductVariantOptions($variant_id),
				'images'     => $this->model_catalog_product->getProductVariantImages($variant_id)
			];
		}
	 ...
	 */
	public function getProductVariantOptions(int $variant_id): array {
		$query = $this->db->query("SELECT `product_option_value_id` FROM `" . DB_PREFIX . "product_variant_option` WHERE `variant_id` = '" . (int)$variant_id . "'");
	 ...
	 * @return array<int, array<string, mixed>>
	 */
	public function getProductVariantImages(int $variant_id): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_variant_image` WHERE `variant_id` = '" . (int)$variant_id . "' ORDER BY `sort_order` ASC, `variant_image_id` ASC");

		$images = [];

		foreach ($query->rows as $row) {
			$images[] = [
				'variant_image_id' => (int)$row['variant_image_id'],
				'image'            => $row['image'],
				'sort_order'       => (int)$row['sort_order']
			];
		}
