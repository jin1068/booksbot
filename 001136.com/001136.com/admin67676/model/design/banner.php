...
class Banner extends \Opencart\System\Engine\Model {
	...
	public function addImage(int $banner_id, int $language_id, array $data): void {
		$image_mobile = isset($data['image_mobile']) ? $this->db->escape($data['image_mobile']) : '';
		
		$this->db->query("INSERT INTO `" . DB_PREFIX . "banner_image` SET `banner_id` = '" . (int)$banner_id . "', `language_id` = '" . (int)$language_id . "', `title` = '" . $this->db->escape($data['title']) . "', `link` = '" . $this->db->escape($data['link']) . "', `image` = '" . $this->db->escape($data['image']) . "', `image_mobile` = '" . $image_mobile . "', `sort_order` = '" . (int)$data['sort_order'] . "'");
	}
