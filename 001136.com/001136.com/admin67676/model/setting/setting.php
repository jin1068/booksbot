...
class Setting extends \Opencart\System\Engine\Model {
	 ...
	 * Edit Setting Value
	 *
	 * Update or create a single key/value pair under a setting code without touching other keys.
	 *
	 * @param string     $code
	 * @param string     $key
	 * @param mixed      $value
	 ...
	 * @return void
	 */
	public function editSettingValue(string $code, string $key, mixed $value, int $store_id = 0): void {
		$is_array = is_array($value);

		if ($is_array) {
			$encoded = json_encode($value, JSON_UNESCAPED_UNICODE);
			$value_string = $encoded !== false ? $encoded : '[]';
		} else {
			$value_string = (string)$value;
		}

		$this->db->query("UPDATE `" . DB_PREFIX . "setting` SET `value` = '" . $this->db->escape($value_string) . "', `serialized` = '" . (int)$is_array . "' WHERE `code` = '" . $this->db->escape($code) . "' AND `key` = '" . $this->db->escape($key) . "' AND `store_id` = '" . (int)$store_id . "'");

		if (!$this->db->countAffected()) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `store_id` = '" . (int)$store_id . "', `code` = '" . $this->db->escape($code) . "', `key` = '" . $this->db->escape($key) . "', `value` = '" . $this->db->escape($value_string) . "', `serialized` = '" . (int)$is_array . "'");
		}
	}
