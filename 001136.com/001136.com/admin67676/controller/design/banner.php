...
class Banner extends \Opencart\System\Engine\Controller {
	...
	public function form(): void {
		...
		foreach ($banner_images as $language_id => $banner_image) {
			foreach ($banner_image as $value) {
				...
				// 处理手机端图片
				if (isset($value['image_mobile']) && $value['image_mobile'] && is_file(DIR_IMAGE . html_entity_decode($value['image_mobile'], ENT_QUOTES, 'UTF-8'))) {
	...
	public function uploadCropped(): void {
		...
		if (!$json && $image_data === '') {
			$json['error'] = 'No image data received.';
		}
		...
		if (!$json) {
			if (strpos($image_data, 'base64,') !== false) {
				[$meta, $payload] = explode('base64,', $image_data, 2);

				if (preg_match('/^data:image\/([\w+-]+)/i', $meta, $matches)) {
		...
		$directory = DIR_IMAGE . 'catalog/banner/';

		if (!$json) {
			if (!is_dir($directory)) {
				if (!@mkdir($directory, 0755, true) && !is_dir($directory)) {
					$json['error'] = 'Unable to create banner directory.';
				}
			}
		}

		if (!$json && !is_writable($directory)) {
			$json['error'] = 'Banner directory is not writable.';
		}

		if (!$json) {
			$filename = sprintf('banner_edited_%s_%04d.%s', gmdate('YmdHis'), mt_rand(0, 9999), $extension);
			$filepath = $directory . $filename;

			if (@file_put_contents($filepath, $binary) === false) {
				$error = error_get_last();
				$json['error'] = 'Failed to save image.';
				$json['debug'] = $error['message'] ?? 'unknown error';
			} else {
				$json['success'] = $this->language->get('text_success');
				$json['image_path'] = 'catalog/banner/' . $filename;

				$catalog_url = defined('HTTPS_CATALOG') ? HTTPS_CATALOG : HTTP_CATALOG;
				$json['image_url'] = $catalog_url . 'image/' . $json['image_path'];
				$json['thumb'] = $this->model_tool_image->resize($json['image_path'], $this->config->get('config_image_default_width'), $this->config->get('config_image_default_height'));
			}
		}
