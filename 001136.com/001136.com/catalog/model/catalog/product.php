...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProducts(array $data = []): array {
		...
		if (!empty($data['filter_search']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_search'])) {
				...
				// 智能空格匹配：搜索去除空格后的产品名称
				$search_no_space = str_replace(' ', '', $data['filter_search']);
				$sql .= " OR REPLACE(`pd`.`name`, ' ', '') LIKE '" . $this->db->escape('%' . $search_no_space . '%') . "'";
	...
	/**
	 * Get Option Meta (Plan B Enhancement)
	 * 获取产品选项的增强元数据
	 *
	 ...
	 */
	public function getOptionMeta(int $product_option_id): array {
		$query = $this->db->query("
			SELECT 
				display_type,
				option_group,
	...
	/**
	 * Get Option Value Meta (Plan B Enhancement)
	 * 获取产品选项值的增强元数据
	 *
	 ...
	 * @return array<string, mixed>
	 */
	public function getOptionValueMeta(int $product_option_value_id): array {
		$query = $this->db->query("
			SELECT 
				image_thumb,
				label,
	...
	/**
	 * Get Option Combination (Plan B Enhancement)
	 * 获取选项组合信息（库存、价格等）
	 *
	 ...
	 * @return array<string, mixed>
	 */
	public function getOptionCombination(int $product_id, array $options): array {
		// 构建选项组合键
		$combination_keys = [];
		foreach ($options as $option_id => $value_id) {
			$combination_keys[] = $option_id . '_' . $value_id;
		}
