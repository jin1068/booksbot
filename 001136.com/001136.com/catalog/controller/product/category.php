...
class Category extends \Opencart\System\Engine\Controller {
	...
	public function index(): ?\Opencart\System\Engine\Action {
		...
		if ($category_info) {
			...
			if ($category_name) {
				$normalized = str_replace(['（', '）'], ['(', ')'], $category_name);
				...
				$normalized_no_space = str_replace(' ', '', $normalized);

				$spec_map = [
					'iphone'                              => 'apple-iphone-series',
					'iphone手机'                            => 'apple-iphone-series',
					'apple iphone'                        => 'apple-iphone-series',
					'苹果iphone'                            => 'apple-iphone-series',
					'三星galaxy'                            => 'samsung-galaxy-s-series',
					'三星 galaxy'                           => 'samsung-galaxy-s-series',
					'samsung galaxy'                      => 'samsung-galaxy-s-series',
					'galaxy s 系列'                         => 'samsung-galaxy-s-series',
					'谷歌pixel'                             => 'google-pixel-series',
					'谷歌 pixel'                            => 'google-pixel-series',
					'google pixel'                        => 'google-pixel-series',
					'pixel 系列'                            => 'google-pixel-series',
					'小米手机'                              => 'other-series-xiaomi',
					'xiaomi'                              => 'other-series-xiaomi',
					'other series - xiaomi'              => 'other-series-xiaomi',
					'其他系列 xiaomi 小米'                 => 'other-series-xiaomi',
					'华为手机'                              => 'other-series-huawei',
					'huawei'                              => 'other-series-huawei',
					'other series - huawei'              => 'other-series-huawei',
					'一加'                                 => 'other-series-oneplus',
					'oneplus'                             => 'other-series-oneplus',
					'other series - oneplus'             => 'other-series-oneplus',
					'nothing'                             => 'other-series-nothing',
					'其他品牌手机'                          => 'other-series-nothing',
					'其他品牌'                              => 'other-series-nothing',
					'other brand phones'                  => 'other-series-nothing',
					'other phones'                        => 'other-series-nothing',
					'other series - nothing'             => 'other-series-nothing',
					'摩托罗拉'                              => 'other-series-motorola',
					'motorola'                            => 'other-series-motorola',
					'other series - motorola'            => 'other-series-motorola',
					'真我'                                 => 'other-series-realme',
					'realme'                              => 'other-series-realme',
					'other series - realme'              => 'other-series-realme',
					'索尼'                                 => 'other-series-sony',
					'sony'                                => 'other-series-sony',
					'other series - sony'                => 'other-series-sony',
					'平板电脑'                              => 'samsung-galaxy-tab-series-tablets',
					'平板系列'                              => 'samsung-galaxy-tab-series-tablets',
					'galaxy tab'                          => 'samsung-galaxy-tab-series-tablets',
					'三星平板'                              => 'samsung-galaxy-tab-series-tablets',
					'tablets'                             => 'samsung-galaxy-tab-series-tablets',
					'tablet pcs'                          => 'samsung-galaxy-tab-series-tablets',
					'ipad'                                => 'apple-ipad-series-tablets',
					'apple ipad'                          => 'apple-ipad-series-tablets',
					'samsung galaxy tab series (tablets)' => 'samsung-galaxy-tab-series-tablets',
					'apple ipad series (tablets)'         => 'apple-ipad-series-tablets'
				];

				$spec_key = $spec_map[$normalized] ?? ($spec_map[$normalized_no_space] ?? null);

				if ($spec_key) {
					$spec_data = new \Opencart\System\Library\SpecData();
					$group = $spec_data->getGroup($spec_key);

					if ($group) {
						$summary = $this->language->get('text_spec_summary') ?? '查看完整规格';

						$data['description'] .= '<details class="category-specs mt-3 mb-4" style="border:1px solid #e6e6e6;border-radius:6px;padding:1rem;background:#fafafa;">';
						$data['description'] .= '<summary style="cursor:pointer;font-weight:600;font-size:1rem;list-style:none;">' . sprintf('%s (%s)', $summary, $category_name) . '</summary>';
						$data['description'] .= '<div class="category-specs__content mt-3">' . $this->load->view('product/specs/table', [
							'spec'             => $group,
			...
			// Get all products in this category to find their manufacturers
			$category_filter_data = [
				'filter_category_id'  => $category_id,
				'filter_sub_category' => true
			];
			
			$category_products = $this->model_catalog_product->getProducts($category_filter_data);
			$manufacturer_ids = [];
			
			foreach ($category_products as $product) {
				if ($product['manufacturer_id'] && !in_array($product['manufacturer_id'], $manufacturer_ids)) {
			...
			foreach ($manufacturer_ids as $manufacturer_id) {
				...
				if ($manufacturer_info) {
					$data['manufacturers'][] = [
						...
						'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $this->request->get['path'] . '&manufacturer_id=' . $manufacturer_id)
					];
				}
			}
