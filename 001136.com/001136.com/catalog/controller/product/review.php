...
class Review extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		...
		$allowed_groups = (array)$this->config->get('config_review_customer_groups');

		if ($allowed_groups) {
			if (!$this->customer->isLogged() || !in_array($this->customer->getGroupId(), $allowed_groups)) {
				$data['review_guest'] = false;
				$restriction_messages[] = $this->language->get('text_review_group_required');
			}
		}

		$minimum_orders = (int)$this->config->get('config_review_min_orders');

		if ($minimum_orders > 0) {
			if (!$this->customer->isLogged()) {
				$data['review_guest'] = false;
				$restriction_messages[] = sprintf($this->language->get('text_review_order_required'), $minimum_orders);
			} else {
				$this->load->model('account/order');

				if ($this->model_account_order->getTotalOrders() < $minimum_orders) {
					$data['review_guest'] = false;
					$restriction_messages[] = sprintf($this->language->get('text_review_order_required'), $minimum_orders);
				}
			}
		...
		$data['review_restriction'] = implode(' ', array_unique($restriction_messages));
	...
	public function write(): void {
		...
		if ($allowed_groups) {
			if (!$this->customer->isLogged() || !in_array($this->customer->getGroupId(), $allowed_groups)) {
				$json['error']['warning'] = $this->language->get('error_review_group');
			}
		}
		...
		if ($minimum_orders > 0) {
			if (!$this->customer->isLogged()) {
				$json['error']['warning'] = $this->language->get('error_review_order');
			} else {
				$this->load->model('account/order');
